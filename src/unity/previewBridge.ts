...
const question = z.object({
  id,
  questionText: requiredText(2000),
  hintText: z.string().max(2000),
  choices: z
    .array(choice)
    .min(2)
    .max(6)
    ...
    .refine(
      (choices) => new Set(choices.map((c) => c.id)).size === choices.length,
      'Answer IDs must be unique',
    ),
})
export const PreviewActivitySchema = z
  .object({
    config: ActivityConfigSchema.omit({ imagePresetIndex: true }).extend({
      activityId: id,
  ...
  .superRefine(({ config, questions }, ctx) => {
    if (config.activityType !== 'Classic' && questions.length < config.pieceCountPreset) {
      ctx.addIssue({
        code: 'custom',
        path: ['questions'],
        message: `Add at least ${config.pieceCountPreset} complete questions for this puzzle`,
      })
    }
  })
export const PreviewBootSchema = z
  .object({
    type: z.literal('preview-boot'),
    ...
    picture: z.object({
      ...
      pngBase64: z
        .string()
        .min(1)
        .max(Math.ceil(MAX_PREVIEW_IMAGE_BYTES / 3) * 4),
    }),
  })
...
export function previewProblems(
  ...
  const problems: string[] = parsed.success
    ...
    : parsed.error.issues.map((issue) => {
        ...
        if (issue.path.includes('questions'))
          return 'Complete each question and answer, choose one correct answer, and provide enough questions for the pieces.'
  ...
  const local =
    ...
    customPicture?.key === 'custom' &&
    customPicture.pngBase64.length > 0 &&
    customPicture.pngBase64.length <= Math.ceil(MAX_PREVIEW_IMAGE_BYTES / 3) * 4
  if (!picture && !local) problems.push('Choose a puzzle picture from the library.')
...
export async function preparePreview(
  ...
  if (customPicture && (draft.meta.imageKey !== 'custom' || customPicture.key !== 'custom'))
    throw new Error('The custom picture does not match this puzzle.')
  const picture = customPicture ?? (await prepareLibraryPicture(draft.meta.imageKey, signal))
  signal.throwIfAborted()
  return PreviewBootSchema.parse({
    type: 'preview-boot',
    previewVersion: 1,
    requestId,
    ...activity,
