...
describe('booting a configured build', () => {
  ...
  it('gives the canvas an id so Unity can resolve its keyboard event target', () => {
    render(<UnityStage activityId="demo" />)
    expect(canvas()?.id).toBe('unity-canvas')
  })

  it('passes Unity the capped device pixel ratio for sharp phone rendering', () => {
    vi.stubGlobal('devicePixelRatio', 3)
    const unity = stubUnityFactory()
    render(<UnityStage activityId="demo" />)
    fireLoad()

    expect(unity.createUnityInstance.mock.calls[0]?.[1]).toMatchObject({ devicePixelRatio: 2 })
  })
